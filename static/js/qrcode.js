// Pure Vanilla JS QR Code Generator Library (Kazuhiko Arase QR Code Spec Engine)
// Generates standard, pixel-perfect, scannable QR codes for UPI URIs (upi://pay?...)
...
(function () {
      ...
      for (var r = 0; r < this.getModuleCount(); r++) {
        for (var c = 0; c < this.getModuleCount(); c++) {
          if (this.isDark(r, c)) {
            ...
            svg += '<rect x="' + x + '" y="' + y + '" width="' + cellSize + '" height="' + cellSize + '" fill="#071A33"/>';
          }
        }
      }
      svg += '</svg>';
  ...
  QRRSBlock.getRSBlocks = function (typeNumber, errorCorrectionLevel) {
    var rsBlock = QRRSBlock.getByTypeNumber(typeNumber, errorCorrectionLevel);
  ...
  QRRSBlock.getByTypeNumber = function (typeNumber, errorCorrectionLevel) {
  ...
  window.AeronexaQR = {
    generateSVG: function (text, cellSize) {
      cellSize = cellSize || 5;
      var qr = new QRCode(0, 1); // Auto type size, Error Correction M
      qr.addData(text);
      qr.make();
